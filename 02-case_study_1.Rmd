...
ggplot(states, aes(long,lat,group=region))+
  ...
  geom_polygon(data = oregon, inherit.aes = T, color="pink",fill="pink") +
  geom_point(data=do_gbif,
             aes(Rana_pretiosa_Baird_._Girard._1853.longitude,
                 Rana_pretiosa_Baird_._Girard._1853.latitude),
             inherit.aes = F, alpha=0.5,size=0.5)+
  coord_quickmap() +
  theme_bw()
```
...
```{r include=FALSE,fig.dim="30%",fig.align='center',fig.cap="Credits: oregonfrog package"}
knitr::include_graphics("images/oregonfrogs_logo.jpeg")
```
...
```{r include=FALSE,fig.dim="50%",fig.align='center',fig.cap="Credits: https://epsg.io/4326"}
...
```{r intro-8, message=FALSE, warning=FALSE, cache=T, paged.print=FALSE}
...
```{r intro-13, message=FALSE, warning=FALSE, paged.print=FALSE}
library(purrr)
...
```{r 02-case-study-1-1, message=FALSE, warning=FALSE, paged.print=FALSE}
require(tidyverse)
require(oregonfrogs)
require(sf)
...
```{r}
frogs <-data.frame(long=do_gbif$Rana_pretiosa_Baird_._Girard._1853.longitude,lat=do_gbif$Rana_pretiosa_Baird_._Girard._1853.latitude)
