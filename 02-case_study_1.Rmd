...
```{r ch2-01-1, eval=TRUE}
library(tidyverse)
library(oregonfrogs)
library(spocc)
```

```{r ch2-01-2,echo=FALSE}
...
The **Universal Transverse Mercator (UTM)** is a common projection is  which preserves local angles and shapes, and divides the Earth into 60 zones of 6 degrees of longitude in width. Each of the zones uses a transverse Mercator projection that maps a region of large north-south extent. [^1]
...
```{r ch2-07,include=FALSE,fig.dim="50%",fig.align='center',fig.cap="Credits: https://epsg.io/4326"}
...
```{r ch2-10, message=FALSE, warning=FALSE, cache=T, paged.print=FALSE}
library(OpenStreetMap)
map <- openmap(c(43.764375,-121.824775),c(43.814821,-121.764923))
OpenStreetMap::plot.OpenStreetMap(map)
```

In this map we project the points location of the frogs.
...
Packages functionality from rgeos,raster,rgdal,terra packages are included in {sf},and {dismod} 
so we don't need to load them.
...
Here we use **wrld_simpl** data from {maptools}, to be able to use the `plot()` function faster, you can install the package or load it because data is included in the `/data` folder as `.RData` format. We can load the `data/case_study_1.RData` to load all the data we need for this workshop, this is already been done at the very beginning of this chapter. In case you'd like to do that separatley, just type: `load("data/case_study_1.RData")` and all data will be loaded in the global envrionment.
      
      
```{r ch2-14-2, eval= FALSE}
library(maptools) # 'maptools' will be retired by the end of 2023,
                  # some functionality will be moved to 'sp'
world <- maptools::data("wrld_simpl")
```


```{r ch2-15}
plot(wrld_simpl,xlim=c(-125,-100),ylim=c(35,65),axes=T)
points(do_gbif2$longitude,do_gbif2$latitude,col="pink")
```
...
```{r ch2-16-1, include=FALSE,eval=TRUE}
...
```{r ch2-17-2, include=TRUE,eval=TRUE,echo=FALSE}
...
```{r ch2-19,eval=TRUE, include=TRUE,echo=FALSE}
