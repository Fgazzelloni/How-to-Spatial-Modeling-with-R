...
There are a number of spatial models that can be used, for Spatial and Spatio-Temporal Modelling, Prediction and Simulation. One is Kriging, while other methods are the Geographically Weighted Regression (GWR) or Inverse Distance Weighting (IDW)[^1]
...
One example can be found in the {gstat} package, which provide the `kringe()` function.[^1]

[^1]: [gstat](https://cran.r-project.org/web/packages/gstat/gstat.pdf)
